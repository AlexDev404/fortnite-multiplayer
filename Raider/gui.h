        ...
        if (ZeroGUI::Window((char*)"Raider", &pos, FVector2D{ 500.0f, 400.0f }, menu_opened))
        {
            if (bListening && HostBeacon)
            {
                ...
                if (ZeroGUI::Button((char*)"Summon FloorLoot", FVector2D{ 100, 25 })) // this isn't a great idea due to no relevancy...
                {
                 ...
                 if (ZeroGUI::Button((char*)"RevivePlayers", FVector2D { 100, 50 })) // this shit dont work!
                {
                     
                     for (int i = 0; i < HostBeacon->NetDriver->ClientConnections.Num(); i++) { 
                         auto client = HostBeacon->NetDriver->ClientConnections[i];
                         auto pawn = (AFortPlayerPawnAthena*)client->PlayerController->Pawn;
                         pawn->bIsDBNO = false;
                         pawn->OnRep_IsDBNO();
                         pawn->SetHealth(30.0f);
